...
<body class="bg-gray-800 text-white relative grid grid-rows-5 grid-cols-3">
    ...
    <header class="fixed bg-purple-500 top-0 left-0 right-0 z-0 grid grid-rows-1 grid-cols-3 ">
        <div class="container mx-auto flex justify-between p-3 col-span-3">
            <h1 class="grid grid-rows-1 col-span-3 text-4xl py-1">Band overview</h1>
    ...
    <main class="grid grid-rows-3 col-span-3 py-1">
        ...
        @section('content')
            <div class="container mx-auto">
                <h2 class="text-2xl font-bold">Edit Band</h2>
                <form method="POST" action="{{ route('bands.update', ['band' => $band->id]) }}">
                    ...
                    <div class="mb-4 text-black">
                        ...
                        <input type="text" id="name" name="name" class="form-input" value="{{ $band->name }}">
                    </div>
                    <div class="mb-4 text-black">
                        <label for="genre" class="block text-sm font-medium text-white">Genre</label>
                        <input type="text" id="genre" name="genre" class="form-input" value="{{ $band->genre }}">
                    </div>
                    <div class="mb-4 text-black">
                        <label for="founded" class="block text-sm font-medium text-white">Founded</label>
                        <input type="text" id="founded" name="founded" class="form-input" value="{{ $band->founded }}">
                    </div>
                    <div class="mb-4 text-black">
                        <label for="active_till" class="block text-sm font-medium text-white">Active_till</label>
                        <input type="text" id="active_till" name="active_till" class="form-input" value="{{ $band->active_till }}">
                    </div>
